...
namespace Drupal\habeuk_minimal_profile;
...
class HabeukProfileApplyConfig {
  ...
  protected static function instaLLNewTheme($themename) {
    ...
    if (!empty($listThemeVisible['wb_universe']))
      \Drupal::logger('habeuk_minimal_profile')->notice("wb_universe vue");
    else
      \Drupal::logger('habeuk_minimal_profile')->notice("wb_universe error vue");
    if (!empty($listThemeVisible[$themename])) {
      ...
      if (empty($listThemesInstalled[$themename])) {
        ...
        if ($themeInstaller->install($theme_list)) {
          ...
          \Drupal::logger('habeuk_minimal_profile')->notice("Theme installé : $themename");
          return true;
        }
        ...
        \Drupal::logger('habeuk_minimal_profile')->notice("Theme deja installé : $themename");
        return true;
      }
    }
    else
      $message = "(pas visible)";
    \Drupal::logger('habeuk_minimal_profile')->warning("Vous devez installer le theme manuellement, erreur  : $themename : $message");
    return false;
  }
